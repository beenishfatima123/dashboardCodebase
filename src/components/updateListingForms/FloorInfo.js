...
import { Container, Grid, Button, Typography, filledInputClasses } from "@mui/material";
...
const FloorInfo = () => {
  ...
  const initialValues = {
    floor_image: data?.floor_image ? data?.floor_image : []
  };
  ...
  const uploadNewImages = async () => {
    ...
    imageData.append("floor_image_len", newFiles.length);
    newFiles?.forEach((item, i) => {
      imageData.append("floor_image[" + i + "]", item.file);
    });
    const response = await axios.put(`${baseUrl}/users/property/${paramFromDetailListing.id}/`, imageData,
      {
        headers: {
          Authorization: `Token ${loggedInObject?.token}`,
        },
    ...
    if (response?.data?.count === 0) {
      toast.success(JSON.stringify("Floor image(s) uploaded successfully."), {
        position: toast.POSITION.BOTTOM_RIGHT,
        progressStyle: { backgroundColor: "#014493" },
      });
    }
