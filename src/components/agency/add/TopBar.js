...
import { createVerificationRequest } from "../../../features/verificationSlice";
...
const TopBar = () => {
  ...
  const { createAgencyData, createAgencyDataApiInfo } = useSelector(
    (state) => state.agency
  );
  ...
  const handleUpdate = () => {
    let inValid = isFormValid(createAgencyData);
    if (!inValid) {
      toast.error(
      ...
      dispatch(
        createAgency({
          ...
          data: getCompanyPostData(createAgencyData, loggedInObject?.id, true),
        })
      );
    }
  };
  ...
  useEffect(() => {
    if (createAgencyDataApiInfo?.response?.status === "True") {
      if (
        createAgencyData?.verification_files &&
        ...
        dispatch(
          createVerificationRequest({
            values: getVerificationData(
              "agency",
              createAgencyDataApiInfo?.response?.result?.id,
              createAgencyData
            ),
  ...
  }, [createAgencyDataApiInfo?.response]);

  useEffect(() => {
    if (createAgencyDataApiInfo?.error) {
